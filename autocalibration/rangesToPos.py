...
def rangesToPos ( R_mis, anchor_ids ):
	...
	if abs(C) <= 1:
		...
		if aux1 < 0:
			raise Exception("Too much noise in the measurements, uncompatible distances (aux1)")
			return
		...
		if aux2 < 0:
			# Debugging error on z3
			print("r01 (origin - y) =", r01)
			print("r02 (origin - x) =", r02)
			print("r03 (origin - z) =", r03)
			print("r12 (y - x)      =", r12)
			print("r13 (y - z)      =", r13)
			print("r23 (x - z)      =", r23)
			
			print("Coordinates (calculated so far) are:")
			print("y1 =", y1)
			print("x2 =", x2)
			print("y2 =", y2)
			print("x3 =", x3)
			print("y3 =", y3)
			print("z3^2 is ", aux2)
			raise Exception("Too much noise in the measurements, uncompatible distances (aux2)")
			return
		...
		anchors = [DeviceCoordinates(anchor_ids[0], 1, Coordinates(0.00, 0.00, 0.00)),
               DeviceCoordinates(anchor_ids[1], 1, Coordinates(0.00, y1, 0.00)),
               DeviceCoordinates(anchor_ids[2], 1, Coordinates(x2, y2, 0.00)),
