...
def pozyx_pose_pub():
	...
	rospy.init_node('pozyx_pose_node_1_tag')
	rate = rospy.Rate(50)
	
	print("Trying to connect to tags...")
	...
	while not rospy.is_shutdown():
		...
		# Preparo una lista contenente la tag che voglio leggere
		pozyx_tag_list = [pozyx_1]
		...
		for pozyx in pozyx_tag_list :
			...
			if status == pypozyx.POZYX_SUCCESS or not (coords.x == 0.0 and coords.y == 0.0 and coords.z == 0.0):
				...
				if not(pozyx == pozyx_0) and not(pozyx == pozyx_1):
					print('No tags functioning this step')
		rate.sleep()
