...
# coding=utf-8
	...
	# 	- pubblicare su un topic la trasformazione da charlie_vicon a charlie_map 
...
def main():
	...
	while not rospy.is_shutdown():
		...
		try:
			p, q = tf_tl.lookupTransform("map", "Charlie", rospy.Time(0))
			
			# preparazione del messaggio
			charlie_vicon_pose = PoseStamped()

			# header
			charlie_vicon_pose.header.frame_id = 'map'
			...
			if antispam == 0:
				antispam = 1
				rospy.loginfo("Started publishing Charlie_vicon_map!")
			
			# t_common = tf_tl.getLatestCommonTime("map", "Charlie")
			# print(p)
			# # charlie_vicon_map = tf_tl.transformPose("/map", charlie_vicon_pose)
			# rospy.loginfo("in vicon %f", charlie_vicon_pose.pose.position.x)
			# # rospy.loginfo("in map %f", charlie_vicon_map.pose.position.x)
			# rospy.loginfo("in map %f", p[0])


		except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
			rospy.logwarn("No Charlie_vicon_map published")
