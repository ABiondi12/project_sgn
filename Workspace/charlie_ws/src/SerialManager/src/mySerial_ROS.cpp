...
int main(int argc, char **argv)
{
	...
	printf("Starting node Serial Manager... \n");
	ros::init(argc, argv, "SerialManager");
  	ros::NodeHandle n;
	...
	printf("Finished setting publishing topics! \n");

	ros::Rate loop_rate(100); 	

	serial_init(); 				//init seriale
	...
	while (ros::ok())
	{
		...
		write(serial_port, a, 1);			// write(serial_port, msg, sizeof(msg));	
...
int serial_init()
{
	printf("Trying to open serial at: /dev/ttyUSBserial \n");
	serial_port = open("/dev/ttyUSBserial", O_RDWR);
...
void Pose_cb_back(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    pos_z_b = msg->pose.position.z;
  	ROS_INFO("Pose_cb_back: pos_x_b: %f pos_y_b: %f pos_z_b: %f \n",pos_x_b, pos_y_b, pos_z_b  );
