%%% FREQUENZA DEL POSITIONING (PROTOCOLLO FAST)
...
% UWB ( con utilizzo del protocollo FAST )e ne effettua un plot dei 
% risultati medi.
%**************************************************************************
...
for t=0:(num_el - 1)
        ...
        n = nrows(1);  % numero righe di una singola cella
        m = length(ncols); % numero colonne
        C = zeros(n,m);
        h=1; % indice di riga
...
%************************ UWB SETTINGS ************************************
% Esempi0 di UWS setting:
%       Ranging protocol: FAST
%       Channel: 5
%       Bitrate: 110 kb/s
%       PRF: 64 MHz
...
% open figure
figure ('name','update rate PROTOCOL FAST')
fig = gcf;
for i =1:8
    % freq_1
...
grid on
title('update rate PROTOCOL FAST')
xlabel('Preamble length')
ylabel('Positioning duration [ms]')
lgb = legend([plot1,plot2,plot3],'bitrate=110 kb/s, PRF=64 MHz',...
