...
def rangesToPos ( R_mis, anchor_ids ):
	print("Vettore delle distanze tra le ancore: \n [r01,r02,r03,r12,r13,r23] =", R_mis)
	print()
	for r in R_mis:
		if r<0 :
			raise Exception("Hai inserito una distanza negativa '\n'")
			return
	r01 = R_mis[0]
	r02 = R_mis[1]
	r03 = R_mis[2]
	...
	if abs(C) <= 1:
		...
		if aux1 < 0:
			raise Exception("Misurazioni tra le ancore affette da troppo rumore")
			return
		...
		raise Exception("Distanze incompatibili, autocalibrazione fallita")
		return
...
def average(x):
	av = 0
	if len(x) == 0:
		raise Exception("Error in anchor distances, check anchors' IDs")
