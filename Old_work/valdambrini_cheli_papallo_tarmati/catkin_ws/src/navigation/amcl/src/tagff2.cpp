...
double angle;
double angle_body;
double beta=0.9; //guadagno del Fading Filter
...
ros::Subscriber sub_or = n.subscribe("/orientation", 1000, orientation_Callback);
 ...
 // pubblicazione sul topic "tag_ff2"
ros::Publisher pubff = n.advertise< geometry_msgs::Point>("/tag_ff2", 1000);

 // pubblicazione sul topic "tag_matlab0_ff2"
ros::Publisher pubtag = n.advertise< geometry_msgs::Point>("/tag_matlab0_ff2", 1000);
...
ff_x=pre_x+x_d_pre*0.2+(1-beta*beta)*(uwb0.x-(pre_x+x_d_pre*0.2)); 
ff_y=pre_y+y_d_pre*0.2+(1-beta*beta)*(uwb0.y-(pre_y+y_d_pre*0.2));
x_d=x_d_pre+(1-beta)*(1-beta)*5*(uwb0.x-(pre_x+x_d_pre*0.2));
y_d=y_d_pre+(1-beta)*(1-beta)*5*(uwb0.y-(pre_y+y_d_pre*0.2));
...
{ 
//utilizzata una matrice di rototraslazione per portare da frame uwb a frame odom
...
//correzione dell'angolo per limitarlo in un intervallo di +- 3.14
 ...
 //pubblicazione trasformata tra odom e base_link
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "odom", "base_link_ff2"));
    
 //pubblicazione della posizione del robot filtrata sul topic tag_ff2
    tag_ff.x=ff_x;
    tag_ff.y=ff_y;
    tag_ff.z=-orien.z;
    pubff.publish(tag_ff);
    
 // pubblicazione del valore della tag0 sul topic tag_matlab0_ff2
