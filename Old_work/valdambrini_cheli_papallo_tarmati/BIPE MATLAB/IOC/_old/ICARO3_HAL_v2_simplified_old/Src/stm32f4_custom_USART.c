...
void USART3_IRQHandler()
{
  ...
  tmp1 = __HAL_UART_GET_FLAG(&Uart3Handle, UART_FLAG_PE);
  tmp2 = __HAL_UART_GET_IT_SOURCE(&Uart3Handle, UART_IT_PE);
  /* UART parity error interrupt occurred -----------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_UART_CLEAR_PEFLAG(&Uart3Handle);
    Uart3Handle.ErrorCode |= HAL_UART_ERROR_PE;
  }

  tmp1 = __HAL_UART_GET_FLAG(&Uart3Handle, UART_FLAG_FE);
  tmp2 = __HAL_UART_GET_IT_SOURCE(&Uart3Handle, UART_IT_ERR);
  /* UART frame error interrupt occurred ------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_UART_CLEAR_FEFLAG(&Uart3Handle);
    Uart3Handle.ErrorCode |= HAL_UART_ERROR_FE;
  }

  tmp1 = __HAL_UART_GET_FLAG(&Uart3Handle, UART_FLAG_NE);
  tmp2 = __HAL_UART_GET_IT_SOURCE(&Uart3Handle, UART_IT_ERR);
  /* UART noise error interrupt occurred ------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_UART_CLEAR_NEFLAG(&Uart3Handle);
    Uart3Handle.ErrorCode |= HAL_UART_ERROR_NE;
  }

  tmp1 = __HAL_UART_GET_FLAG(&Uart3Handle, UART_FLAG_ORE);
  tmp2 = __HAL_UART_GET_IT_SOURCE(&Uart3Handle, UART_IT_ERR);
  /* UART Over-Run interrupt occurred ---------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_UART_CLEAR_OREFLAG(&Uart3Handle);
    Uart3Handle.ErrorCode |= HAL_UART_ERROR_ORE;
  }

  if(Uart3Handle.ErrorCode != HAL_UART_ERROR_NONE)
  {
    /* Set the UART state ready to be able to start again the process */
    Uart3Handle.State = HAL_UART_STATE_READY;
    
    //do not call callback
		//HAL_UART_ErrorCallback(&Uart2Handle);
  }
