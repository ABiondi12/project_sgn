...
/* USER CODE BEGIN 0 */
#include <stdio.h>
#include "ICARO3_Onda_bipe.h"          /* Model's header file */
...
/* Defined in ICARO3_Onda_bipe.c file */
extern void ICARO3_Onda_bipe_SetEventsForThisBaseStep(boolean_T*);

/* Flags for taskOverrun */
static boolean_T OverrunFlags[5];
...
mw_semaphore_t subrateTaskSem[(5 - 1)];//see above
int taskId[(5)];                       //id of sub rate tasks (id of base rate task is 0
mw_thread_t schedulerThread;
mw_thread_t baseRateThread;
unsigned long threadJoinStatus[(5 - 1)*2];
int terminatingmodel = 0;
mw_thread_t subRateThread[(5 - 1)];
...
//int32_t SubRates_stack_size[(5 - 1)] = {768, 256, 256, 128};
...
UBaseType_t SubRate_emptystacksize[(5 - 1)];
void subrateTask(void *arg)
{
  int tid = *((int *) arg);
  int subRateId;
  ...
  while (runModel) {
    ...
    ICARO3_Onda_bipe_step(subRateId);
...
{
  int_T i;
  runModel = (rtmGetErrorStatus(ICARO3_Onda_bipe_M) == (NULL));
  while (runModel) {
    ...
    for (i = 1; i <= (5 - 1); i++) {
      if (rtmStepTask(ICARO3_Onda_bipe_M, i)) {
...
void exitTask(int sig)
{
  rtmSetErrorStatus(ICARO3_Onda_bipe_M, "stopping the model");
}
...
int main (void)
{
  ...
  /* Data initialization */
  int_T i;
  boolean_T eventFlags[5];             /* Model has 5 rates */
  ...
  traceSTART();
  mw_RTOSInit(0.001, (5 - 1), BaseRate_stack_size, SubRates_stack_size );
  ...
  while (1) {
    ...
    if (remainAutoReloadTimerLoopVal_S == 0) {
      ...
      if (OverrunFlags[0]) {
        rtmSetErrorStatus(ICARO3_Onda_bipe_M, "Overrun");
      }
       ...
       */
      ICARO3_Onda_bipe_SetEventsForThisBaseStep(eventFlags);
