...
//USART 2
#define USART2_BAUD_RATE 57600
// 9600 è per il sensore ad ultrasuoni. Per il GPS ublox settare il baud a115200
...
//USART 4
#define USART4_BAUD_RATE 57600      //For Aerocom AC4868
...
/* NEW: */
UART_HandleTypeDef 			Uart2Handle; //note uart 4 and 5 are UART type, while the others (1,2,3,6) are USART type
UART_HandleTypeDef 			Uart3Handle;
UART_HandleTypeDef 			Uart4Handle;		
UART_HandleTypeDef 			Uart5Handle;


//counters for ORE events
//	ORE_count[N] is for USART_N
unsigned int ORE_count[7] = {0, 0, 0, 0, 0, 0, 0};  
...
{		
  ...
  tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_PE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, USART_IT_PE);
  /* USART parity error interrupt occurred -----------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_PEFLAG(&Uart2Handle);
    Uart2Handle.ErrorCode |= HAL_USART_ERROR_PE;
  }

  tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_FE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, USART_IT_ERR);
  /* USART frame error interrupt occurred ------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_FEFLAG(&Uart2Handle);
    Uart2Handle.ErrorCode |= HAL_USART_ERROR_FE;
  }

  tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_NE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, USART_IT_ERR);
  /* USART noise error interrupt occurred ------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_NEFLAG(&Uart2Handle);
    Uart2Handle.ErrorCode |= HAL_USART_ERROR_NE;
  }

  tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_ORE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, UART_IT_RXNE); // it was USART_IT_ERR);
  /* USART Over-Run interrupt occurred ---------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_OREFLAG(&Uart2Handle);
		ORE_count[2]++; //use index 2 for USART2
    Uart2Handle.ErrorCode |= HAL_USART_ERROR_ORE;
  }

  if(Uart2Handle.ErrorCode != HAL_USART_ERROR_NONE)
  {
    /* Set the USART state ready to be able to start again the process */
    Uart2Handle.State = HAL_UART_STATE_READY;
    
    //do not call callback
		//HAL_USART_ErrorCallback(&Uart2Handle);
  }

//  tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_RXNE);
//  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, USART_IT_RXNE);
//  /* USART in mode Receiver --------------------------------------------------*/
//  if((tmp1 != RESET) && (tmp2 != RESET))
//  {
//    if(&Uart2Handle->State == HAL_USART_STATE_BUSY_RX)
//    {
//      USART_Receive_IT(&Uart2Handle);
//    }
//    else
//    {
//      USART_TransmitReceive_IT(&Uart2Handle);
//    }
//  }

//  tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_TXE);
//  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, USART_IT_TXE);
//  /* USART in mode Transmitter -----------------------------------------------*/
//  if((tmp1 != RESET) && (tmp2 != RESET))
//  {
//    if(&Uart2Handle->State == HAL_USART_STATE_BUSY_TX)
//    {
//      USART_Transmit_IT(&Uart2Handle);
//    }
//    else
//    {
//      USART_TransmitReceive_IT(&Uart2Handle);
...
//  }
		...
		tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, UART_FLAG_RXNE);
		tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, UART_IT_RXNE);
		...
		tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, UART_FLAG_TXE);
		tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, UART_IT_TXE);
      ...
      if(CircularBuffer_Read(&USART2_TX_CB, &item))
			...
			__HAL_USART_DISABLE_IT(&Uart2Handle, UART_IT_TXE);
					
    }
	
		
		
//		 tmp1 = __HAL_USART_GET_FLAG(&Uart2Handle, USART_FLAG_TC);
//  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart2Handle, USART_IT_TC);
//  /* USART in mode Transmitter (transmission end) ----------------------------*/
//  if((tmp1 != RESET) && (tmp2 != RESET))
//  {
//    USART_EndTransmit_IT(&Uart2Handle);
//  }
...
void USART3_IRQHandler()
{
  ...
  tmp1 = __HAL_USART_GET_FLAG(&Uart3Handle, USART_FLAG_ORE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart3Handle, UART_IT_RXNE); // it was USART_IT_ERR);
  /* USART Over-Run interrupt occurred ---------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_OREFLAG(&Uart3Handle);
		ORE_count[3]++; //use index 3 for USART2
    Uart3Handle.ErrorCode |= HAL_USART_ERROR_ORE;
  }

  if(Uart3Handle.ErrorCode != HAL_USART_ERROR_NONE)
  {
    /* Set the USART state ready to be able to start again the process */
    Uart3Handle.State = HAL_UART_STATE_READY;
    ...
    //do not call callback
        ...
        //OLD: if(UART_GetITStatus(UART2, UART_IT_RXNE) != RESET)
		if((tmp1 != RESET) && (tmp2 != RESET))		{
				// RX interrupt: new data received from UART
        CircularBuffer_Write(&USART3_RX_CB, Uart3Handle.Instance->DR); //OLD: UART2->DR);
		}
		
		/* UART1 in mode Transmitter ------------------------------------------------*/
		tmp1 = __HAL_UART_GET_FLAG(&Uart3Handle, UART_FLAG_TXE);
		tmp2 = __HAL_UART_GET_IT_SOURCE(&Uart3Handle, UART_IT_TXE);
		//OLD: if(UART_GetITStatus(UART2, UART_IT_TXE) != RESET)
		if((tmp1 != RESET) && (tmp2 != RESET)){
			// TX interrupt: data correctly transmitted 
			uint8_t item;
      /* Check if there are data to be transmitted, otherwise disable
      ...
      if(CircularBuffer_Read(&USART3_TX_CB, &item))
            Uart3Handle.Instance->DR = item; //OLD. UART2->DR = item;
      else
			// TX buffer is empty => Disable the UART Transmit data register empty Interrupt
...
void UART4_IRQHandler()
{
  ...
  tmp1 = __HAL_USART_GET_FLAG(&Uart4Handle, USART_FLAG_ORE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart4Handle, UART_IT_RXNE); // it was USART_IT_ERR);
  /* USART Over-Run interrupt occurred ---------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_OREFLAG(&Uart4Handle);
		ORE_count[4]++; //use index 4 for USART4
    Uart4Handle.ErrorCode |= HAL_USART_ERROR_ORE;
  }

  if(Uart4Handle.ErrorCode != HAL_USART_ERROR_NONE)
  {
    /* Set the USART state ready to be able to start again the process */
    Uart4Handle.State = HAL_UART_STATE_READY;
...
void UART5_IRQHandler()
{
  ...
  tmp1 = __HAL_USART_GET_FLAG(&Uart5Handle, USART_FLAG_ORE);
  tmp2 = __HAL_USART_GET_IT_SOURCE(&Uart5Handle, UART_IT_RXNE); // it was USART_IT_ERR);
  /* USART Over-Run interrupt occurred ---------------------------------------*/
  if((tmp1 != RESET) && (tmp2 != RESET))
  {
    __HAL_USART_CLEAR_OREFLAG(&Uart5Handle);
		ORE_count[5]++; //use index 4 for USART4
    Uart5Handle.ErrorCode |= HAL_USART_ERROR_ORE;
  }

  if(Uart5Handle.ErrorCode != HAL_USART_ERROR_NONE)
  {
    /* Set the USART state ready to be able to start again the process */
    Uart5Handle.State = HAL_UART_STATE_READY;
