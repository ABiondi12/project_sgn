...
Questo consente di comunicare, appunto, in seriale con la STM e di trasmettere informazioni necessarie all'algoritmo di navigazione.
Per leggere cosa viene trasmesso, rifarsi alla sez.~\ref{sez:L'esperimento}, mentre per sapere che funzione hanno i pin 
consultare datasheet o pi\`u comodamente fig.~\ref{fig: pinout usbserial}. 
A questo dispositivo \`e stato associato il symlink \texttt{ttyUSBserial} per evitare che l’ordine di inizializzazione delle porte usb della raspberry non influenzasse
il nome del collegamento (per esempio: \texttt{ttyUSB0} oppure  \texttt{ttyUSB1})
Su come si realizzi tale symlink abbiamo riportato la procedura dentro il file \texttt{/Info/renaming\_ttyUSB.txt}.


Ultimo elemento da evidenziare \`e la piccola PCB che consente di utilizzare, alternativamente alla STM, un radiocomando per il controllo di sterzo e acceleratore.
...
Abbiamo scritto tutto il necessario nel file: \texttt{Info/README\_wifi\_settings.txt} e adesso lo riportiamo brevemente anche qui.
...
Quello che  vogliamo fare \`e avere il RosMaster su raspberry e utilizzare il pc per tool grafici, esempio rviz o rqt. 
