...
Per prima cosa spostarsi quindi accedere all'applicazione ``Vicon Tracker 3.7.0 x64'' e collocarsi nella tab \textbf{CALIBRAZIONE}?. 
Premere su ?? avviando la procedura di autocalibrazione: spostare la wand con pattern circolari all'interno della stanza assicurandosi di avere sempre pi\`u camere ``a vista''. 
...
Selezioniamo quindi il marker all'incrocio come origine e gli altri marker come asse x e asse y e abbiamo ricalibrato il sistema. 
Nella stanza volo il sistema di riferimento pi\`u usato \`e rappresentato in fig.REF.
\textbf{FARE IMMAGINEEE}

\textbf{NON MI RICORDOOO BENE RIVEDERE!!}
