...
Successivamente, sfruttando la presenza del Vicon nella stanza del volo, abbiamo deciso di effettuare un esperimento classico, vedere sez.~\ref{sez: Guida breve all'esperimento}, in modo tale 
...
\textbf{QUESTO PARAGRAFO MAGARI LO SPOSTIAMO NELLA SEZIONE POZYX?}Durante il nostro lavoro, infatti, erano state riscontrate alcune anomalie, corrette andando a modificare la gestione degli ID nell'esecuzione di alcune funzioni di \verb|positioning| del sistema Pozyx, dove è necessario porre attenzione affinché chi è demandato di eseguirle, tra gli elementi del sistema, abbia al suo interno effettivamente tutte le informazioni corrette e aggiornate di cui necessita. 
...
Infatti, nel caso in cui l'ancora4 venga allontanata troppo dall'origine, l'algoritmo algebrico di autocalibrazione fallisce (si ha problemi di radici di numeri negativi).
